...
// 2. Cria as fitas e bordas dinamicamente e já as acumula
        For i In {1:144}
            idSuper = 10000 + i;
            idEdge1 = 20000 + i;
            idEdge2 = 30000 + i;
            Super~{i}  = Region[ idSuper ];
            Edge1_~{i} = Region[ idEdge1 ];
            Edge2_~{i} = Region[ idEdge2 ];
        ...
        ElseIf(MaterialType == 2)
            // Se for simular como cobre, atribui todas as fitas
            Copper += Region[ {Super} ];
        EndIf
...
Function{
    ...
    // Excitation
    DefineConstant [IblockMax = {430, Name "Input/4Source/0Corrente máxima por bloco (A)"}];
...
Function{
    ...
    I[] = 36 * IblockMax * Sin[2.0 * Pi * f * $Time]; // Corrente total equivalente (36 blocos)
...
{ Name t_space; Type Form0;
        BasisFunction {
            ...
            // Declaração automática das funções de base modais de corrente (uma por fita)
            For i In {1:144}
        ...
        GlobalQuantity {
            ...
            EndFor
            For b In {1:36}
                iRef = tapesPerBlock*(b-1) + 1;
                { Name Vb~{b} ; Type AssociatedWith ; NameOfCoef Ti~{iRef} ; }
            EndFor
        }
    }
    }
...
Formulation {
    ...
    { Name MagDyn_ta; Type FemEquation;
        Quantity {
            ...
            // Mapeia os potenciais globais de corrente como incógnitas
            For i In {1:144}
                { Name T~{i}; Type Global; NameOfSpace t_space[T~{i}]; }
            EndFor

            For b In {1:36}
                { Name Vb~{b}; Type Global; NameOfSpace t_space[Vb~{b}]; }
            EndFor
...
// ====================================================================
            // MODELO DE CIRCUITO ACOPLADO (TENSÃO E KCL POR BLOCO)
            // ====================================================================
            // 1. Acoplamento de Faraday: uma tensão Vb para cada bloco (4 fitas por bloco)
            For b In {1:36}
                For k In {1:tapesPerBlock}
                    i = tapesPerBlock*(b-1) + k;
                    GlobalTerm { [ - $DTime * Dof{Vb~{b}} , {T~{i}} ] ; In Edge1_~{i} ; }
                EndFor
            EndFor

            // 2. Lei de Kirchhoff por bloco (KCL): soma das 4 fitas = corrente imposta do bloco
            For b In {1:36}
                For k In {1:tapesPerBlock}
                    i = tapesPerBlock*(b-1) + k;
                    GlobalTerm { [ Dof{T~{i}} , {Vb~{b}} ] ; In Edge1_~{i} ; }
                EndFor
                iRef = tapesPerBlock*(b-1) + 1;
                GlobalTerm { [ -I[]/36 , {Vb~{b}} ] ; In Edge1_~{iRef} ; }
            EndFor
            // ====================================================================
            // ====================================================================
        ...
        Quantity {
            ...
            { Name V;
                Value{
                    For b In {1:36}
                        iRef = tapesPerBlock*(b-1) + 1;
                        Term{ [ {Vb~{b}} / 36 ] ; In Edge1_~{iRef};}
                    EndFor
                }
            }
            { Name I; // Corrente total recuperada pela fonte (I[])
                Value{ Term{ [ I[] ] ; In Edge1;} }
            }
            { Name dissPowerGlobal;
                Value{
                    // Potência total = soma_b(Vb * Ibloco)
                    For b In {1:36}
                        iRef = tapesPerBlock*(b-1) + 1;
                        Term{ [ thickness[] * {Vb~{b}} * I[]/36 ] ; In Edge1_~{iRef};}
                    EndFor
                }
            }
